"""
Her Travel Map — Telegram Bot (with Cloudinary photo storage)
"""
...
async def help_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "• /add — add a place (city, dream or visited, note, photo)\n"
        "• /list — see everything\n"
        "• /dreams — dream destinations 🌙\n"
        "• /visited — places you've been ✓\n\n"
        "Photos are stored in the cloud so you never lose them 📷",
        parse_mode="Markdown"
    )
...
async def ask_status(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if geo is None:
        await update.message.reply_text(
            "I couldn't find that place. Try a different spelling?\n\nOr /cancel to stop."
        )
        return ASK_CITY
...
async def ask_photo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📷 Want to add a photo? Send one now, or /skip to finish.")
    return ASK_PHOTO
...
async def finish_with_photo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    msg = f"{emoji} *{geo['name']}* added to your travel map!\n_{geo['country']}_"
    if photo_url:
        msg += "\n📷 Photo saved!"
